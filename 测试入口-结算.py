...
from core.常量与路径 import 默认资源路径, 取个人资料路径 as _取个人资料路径
...
def _重建沙盒目录(项目根: str, 模式键: str, 起始等级: int, 起始经验比例: float) -> str:
    ...
    个人资料路径 = _取个人资料路径(沙盒根目录)
    _写入json(个人资料路径, _构建个人资料数据(模式键, 场景起始进度))
    _覆盖存储作用域(_歌曲记录存储作用域, {}, 根目录=沙盒根目录)
    return 沙盒根目录
