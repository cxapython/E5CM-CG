...
def 构建返回选歌动作(载荷: dict | None, 状态: dict | None) -> dict:
    ...
    return {
        ...
        "选歌收藏夹模式": bool(选歌上下文["选歌收藏夹模式"]),
        "选歌收藏夹页码": int(选歌上下文["选歌收藏夹页码"]),
    }
...
def 执行返回选歌(状态: dict | None, 载荷: dict | None, 动作: dict | None = None) -> dict:
    if isinstance(状态, dict):
        try:
            ...
            状态["选歌_恢复收藏夹模式"] = bool(选歌上下文["选歌收藏夹模式"])
            状态["选歌_恢复收藏夹页码"] = int(选歌上下文["选歌收藏夹页码"])
        except Exception:
            pass
...
def _解析选歌上下文(载荷: dict | None, 状态: dict | None, 动作: dict | None) -> dict:
    ...
    恢复收藏夹模式 = _取首个布尔值(
        False,
        动作.get("选歌收藏夹模式", None),
        动作.get("选歌恢复收藏夹模式", None),
        载荷.get("选歌收藏夹模式", None),
        载荷.get("选歌恢复收藏夹模式", None),
        状态.get("选歌_恢复收藏夹模式", None),
        加载页载荷.get("选歌收藏夹模式", None),
        加载页载荷.get("选歌恢复收藏夹模式", None),
    )

    恢复收藏夹页码 = _取首个整数(
        0,
        动作.get("选歌收藏夹页码", None),
        动作.get("选歌恢复收藏夹页码", None),
        载荷.get("选歌收藏夹页码", None),
        载荷.get("选歌恢复收藏夹页码", None),
        状态.get("选歌_恢复收藏夹页码", None),
        加载页载荷.get("选歌收藏夹页码", None),
        加载页载荷.get("选歌恢复收藏夹页码", None),
    )
