...
from core.对局状态 import 取每局所需信用, 初始化对局流程
...
def 主函数():
    ...
    if bool(启动调试设置.get("显示启动幻灯片", 显示启动界面)):
        _播放开场幻灯片(开场动画目录)
    ...
    def _加载全局设置():
        ...
        try:
            当前投币数 = max(0, int(数据.get("投币数",999) or 0))
        except Exception:
            当前投币数 = 0
        ...
        状态["显示启动幻灯片"] = bool(
            数据.get("显示启动幻灯片", 状态.get("显示启动幻灯片", 显示启动界面))
        )
        状态["显示谱面开场动画"] = bool(
        ...
        状态["diy_songs路径"] = str(
            数据.get("diy_songs路径", 状态.get("diy_songs路径", ""))
            or ""
        ).strip()
        状态["疯狂_songs路径"] = str(
            数据.get("疯狂_songs路径", 状态.get("疯狂_songs路径", ""))
            or ""
        ).strip()
        状态["club_songs路径"] = str(
            数据.get("club_songs路径", 状态.get("club_songs路径", ""))
            or ""
        ).strip()
    ...
    def _执行非游戏菜单选项(索引: int):
        nonlocal 投币快捷键录入中, 非游戏菜单开启, 待切换目标场景名, 待切换载荷
        菜单项 = _取非游戏菜单项()
        if not 菜单项:
            return
